...
## 2026-08-21 개인 성적표 2페이지 전환

- `★ [DB]기업맞춤-1차 훈련생성장현황 평가결과 종합.xlsx`의 확정 원점수를 기준으로 `data-growth-detail.js`를 추가했다.
- 관리자 1페이지는 요청한 13개 열의 훈련생별 성장 종합표로 정리하고, 산정 기준·동료평가·사후관리 묶음은 관리자 2페이지로 이동했다.
- 김O교는 `80% 이상수료(취업)`, 김O수는 `수료(취업)`으로 표시한다.
- 개인 성적표는 기존 요약 1장 뒤에 11과목 상세 성적표 2장을 추가한다. 사전진단·본평가·셀프점검은 원자료 값만 표시한다.
- 원자료에 없는 과목별 교수자 서술, 수행준거 상·중·하, 평가상세 점수는 임의 생성하지 않고 `원자료 없음 · 확인 필요`로 표시한다.
- 셀프점검 원자료는 영상·출판 1~6만 존재한다. 광고·브랜딩과 프로젝트1·2·3은 `미실시`로 구분하며, 실제 0점은 0점으로 보존한다.
- 추가 원점수 폴더 `https://drive.google.com/drive/folders/17w6J5UwvTjB5tVFSWuiCGENoxqn5tjIR`에서 11과목 개인 평가 PDF 124건을 확인했다.
- `data-growth-pdf-evidence.js`에 성취수준 124건과 확인 가능한 교수자 의견 121건을 마스킹명 기준으로 반영했다. 실명과 Drive 파일 ID는 공개 저장소 데이터에 넣지 않았다.
- 영상 3건(김O림·문O권·배O연)은 PDF에 별도 서술형 교수자 의견이 확인되지 않아 `PDF 원문 확인 필요`로 남겼다.
- 벡터드로잉 2차 재시험자 4명은 하위 `2차 재시험자_0309` 폴더의 최종 PDF를 다시 확인했다. 김O림 81, 노O진 77, 문O권 67, 유O령 75로 엑셀 최종점수와 일치한다.
- 124개 PDF의 `평가내용` 이후 전문을 `evaluationFullText`로 저장해 과목별 `평가상세·수행준거 전체보기`에서 확인할 수 있게 했다.
- 개인 요약의 비교 그래프 용어를 `영상교과목 / 출판교과목(1~7단위) / 프로젝트1 / 프로젝트2 / 프로젝트3 / 종합`으로 통일했다.
- 레이더 차트는 `영상교과목 / 출판교과목 / 프로젝트1,2,3 / 셀프체크 / 출석` 5개 축으로 정리했다.
- 평가 결과 요약에서 중복 `본평가` 행을 삭제하고 종합행의 첫 두 칸을 병합했다. 주요 평가 지표에서도 `본(재)평가평균`을 삭제했다.
- 하단의 자동 생성 `강점 / 보완점 / 종합 의견`을 모두 삭제하고, 확정 프로젝트1 동료평가·자기평가·교수자협업 3개 점수의 막대 비교와 평균으로 교체했다.
- 개인 성적표 Apps Script 배포 URL: `https://script.google.com/macros/s/AKfycbzJaCGpSMHkMJ4fO9tf9ev2sTsaCLWpS1_g05-p5Vnpr2E-kLpPNW4zorTwxrF5g1CG/exec`
...
# 2026-08-21 개인 성적표 발송 우선 운영

- 시간 제약으로 `growth.html?theme=white&admin=1`의 성장 종합 결과 1~2페이지는 임시 비공개 처리했다.
- 관리자 주소에는 현재 `성장 종합 결과 준비 중입니다.` 안내 1페이지만 표시된다.
- `growth.html?student=...` 개인별 성적표 2페이지와 Apps Script 이메일 발송 로직은 그대로 유지한다.
- 종합 결과 원본 렌더링 코드는 삭제하지 않았다. 재공개할 때 `student-report-html/growth.js`의 `renderGrowthApp()` 관리자 분기에서 `renderGrowthAdminPreparing()`을 `renderGrowthPage()`로 되돌리고 캐시 버전을 갱신한다.
- 현재 캐시/발송 버전: `20260821-personalonly1`
