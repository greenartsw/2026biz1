const CONFIG = {
  ...
  requireCoverSelection: false,
  feedbackSpreadsheetId: '1zZZrmg9d8Tw8CbRGbMMOu5_uGb4cfVgk6WYJ-4ZCTKw',
  feedbackSheetName: '훈련생 피드백',
  feedbackSpreadsheetUrl: 'https://docs.google.com/spreadsheets/d/1zZZrmg9d8Tw8CbRGbMMOu5_uGb4cfVgk6WYJ-4ZCTKw/edit',
...
function doGet(e) {
  ...
  return HtmlService
    .createHtmlOutput(buildProject3DashboardHtml_())
...
function releaseStudent_(rowNumber, testOnly) {
  ...
  try {
    ...
    if (CONFIG.requireCoverSelection !== false && !coverSelection.completed) {
    ...
    const coverAccessUrl = CONFIG.requireCoverSelection === false ? '' : buildStudentCoverAccessUrl_(token, item.team);
...
function buildDashboardRow_(values, rowNumber, roster, scoreMap) {
  ...
  return {
    ...
    companyMemo: cell_(values, CONFIG.feedbackColumns.companyMemo),
...
function getCoverSelectionStatus_() {
  if (CONFIG.requireCoverSelection === false) {
    return { completed: true, teams: [], records: [] };
  }
  const spreadsheet = SpreadsheetApp.openById(CONFIG.feedbackSpreadsheetId);
...
function rowToRecord_(headers, row) {
  ...
  record.companyMemo = firstValue_(record, ["companyMemo", "기업 메모", "기업메모"]);
  return record;
}
...
function buildStudentReportUrl_(studentRouteName) {
  const separator = CONFIG.reportBaseUrl.indexOf('?') === -1 ? '?' : '&';
  return `${CONFIG.reportBaseUrl}${separator}student=${encodeURIComponent(studentRouteName)}&view=both&theme=white`;
}
...
function render(data){data=data||{};var rowsData=data.rows||[];var testRecipient=data.testRecipient||'-';var mode=document.getElementById('mode');mode.className='mode ready';mode.textContent='정식 릴리즈 모드: 학생 메일 발송은 실제 학생에게 발송됩니다. 나에게 테스트는 '+testRecipient+' 에게만 발송됩니다.';document.getElementById('summary').innerHTML='<div class="card"><span class="muted">접수 건수</span><strong>'+esc(rowsData.length)+'</strong></div><div class="card"><span class="muted">운영 방식</span><strong>개인별 피드백</strong></div><div class="card"><span class="muted">발송 모드</span><strong>정식</strong></div><div class="card"><span class="muted">테스트 수신자</span><strong>'+esc(testRecipient)+'</strong></div>';if(!rowsData.length){document.getElementById('table').innerHTML='<div class="empty">접수된 피드백이 없습니다.</div>';setMessage('');return}var rows=rowsData.map(function(row){var rosterNote=row.rosterNote||'';var scoreStatus=row.scoreStatus||'';var releaseStatus=row.releaseStatus||'';var blocked=releaseStatus==='학생발송완료'||releaseStatus==='보류'||rosterNote.indexOf('중탈')!==-1||scoreStatus.indexOf('중탈')!==-1;var releaseLabel=releaseStatus||'학생발송대기';var releaseClass=releaseLabel.indexOf('완료')!==-1?'ok':(releaseLabel==='보류'?'block':'');var holdLabel=releaseStatus==='보류'?'보류 해제':'발송 보류';var holdClass=releaseStatus==='보류'?'':'warn';var rowNumber=Number(row.rowNumber);var scoreLabel=row.overallScore||row.projectScore||'';return '<tr><td>'+esc(row.rowNumber)+'</td><td>'+esc(row.submittedAt)+'</td><td><strong>'+esc(row.trainee)+'</strong><br><span class="student-email">'+esc(row.studentEmail||'이메일 없음')+'</span></td><td>'+esc(row.company)+'<br><span class="muted">'+esc(row.projectName||'프로젝트3')+'</span></td><td>'+esc(scoreLabel)+'</td><td class="feedback">'+esc(row.overallFeedback)+'</td><td><span class="pill">'+esc(row.adminStatus||'관리자알림대기')+'</span><br><span class="pill '+releaseClass+'">'+esc(releaseLabel)+'</span></td><td class="actions"><button '+(blocked?'disabled':'')+' onclick="releaseTestRow('+rowNumber+')">나에게 테스트</button> <button class="primary" '+(blocked?'disabled':'')+' onclick="releaseRow('+rowNumber+')">학생 메일 발송</button> <button class="'+holdClass+'" onclick="toggleHoldRow('+rowNumber+')">'+holdLabel+'</button>'+(row.releaseError?'<div class="error">'+esc(row.releaseError)+'</div>':'')+'</td></tr>'}).join('');document.getElementById('table').innerHTML='<table><thead><tr><th>행</th><th>제출시각</th><th>훈련생</th><th>기업/프로젝트</th><th>만족도</th><th>종합 피드백</th><th>상태</th><th>처리</th></tr></thead><tbody>'+rows+'</tbody></table>';setMessage('로드 완료')}
function releaseTestRow(rowNumber){if(!confirm('이 행을 나에게 테스트 발송할까요? 실제 학생에게는 발송하지 않습니다.'))return;setMessage('테스트 발송 중...');google.script.run.withSuccessHandler(function(message){setMessage(message);loadRows()}).withFailureHandler(showError).releaseStudentTest(rowNumber)}
function releaseRow(rowNumber){if(!confirm('이 학생에게 실제 메일을 발송할까요?'))return;setMessage('학생 메일 발송 중...');google.script.run.withSuccessHandler(function(message){setMessage(message);loadRows()}).withFailureHandler(showError).releaseStudent(rowNumber)}
